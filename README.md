...
1. Download or clone this repository from `https://github.com/filipelinsduarte/scroll-stop`.
...
If Chrome still shows an older orange icon, click **Reload** on the Scroll Stop card in `chrome://extensions`. If it remains cached, remove the old unpacked copy and load this folder again. Version 1.4.1 uses new blue icon filenames specifically to clear the old cache.
...
- Click **Set focus** to define up to five objectives.
...
The challenge copy uses the website that triggered the interruption. For example, attempting to open X displays X in both reflection prompts, not LinkedIn.
